...
import com.kratonsolution.cis.dm.Education;
...
import com.kratonsolution.cis.ui.form.EducationForm;

/**
 * @author Agung Dodi Perdana
 * @email agung.dodi.perdana@gmail.com 
 */
public class EducationView extends DashboardContent
{
	...
	public EducationView(Employee employee)
	{
		super();
	...
	private void initToolbar(Employee employee)
	{
		...
		create.addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				Education step = new Education();
				step.setId("0");
				step.setEmployee(employee);
				
				canvas.getChildren().clear();
				
				EducationForm form = new EducationForm(step);
							...
							for(Object object:grid.getRows().getChildren())
							{
								...
								if(row.getFirstChild() instanceof Checkbox)
								{
									...
									if(check.isChecked())
									{
										...
										if(on != null)
										{
											Iterator<Education> iterator = on.getEducations().iterator();
											while (iterator.hasNext())
											{
												Education edu = (Education) iterator.next();
												if(edu.getId().equals(RowUtils.id(row)))
													iterator.remove();
											}
	...
	private void initGrid(Employee employee)
	{
		...
		grid.getColumns().appendChild(new Column("JENIS",null,"85px"));
		grid.getColumns().appendChild(new Column("PERGURUAN TINGGI",null,"250px"));
		grid.getColumns().appendChild(new Column("TAHUN MULAI",null,"100px"));
		grid.getColumns().appendChild(new Column("TAHUN SELESAI",null,"100px"));
		grid.getColumns().appendChild(new Column(null,null,"25px"));
		grid.getColumns().appendChild(new Column());
		grid.getColumns().getLastChild().setVisible(false);
		grid.setSpan("2");
		
		for(Education fam:service.find(employee.getId()).getEducations())
		{
			A com = new A();
			...
			row.appendChild(new Label(fam.getLevel().name()));
			row.appendChild(new Label(fam.getName()));
			row.appendChild(new Label(fam.getYearStart().toString()));
			row.appendChild(new Label(fam.getYearEnd().toString()));
			row.appendChild(com);
			...
			row.addEventListener(Events.ON_DOUBLE_CLICK, new EventListener<Event>()
			{
				...
				{
					canvas.getChildren().clear();
					EducationForm form = new EducationForm(fam);
					form.setCanvas(canvas);
