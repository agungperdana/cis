...
import com.shara.hr.ui.form.DecreeForm;

/**
 * @author Agung Dodi Perdana
 * @email agung.dodi.perdana@gmail.com 
 */
public class DecreeView extends DashboardContent
{
	...
	public DecreeView(Employee employee)
	{
		super();
	...
	private void initToolbar(Employee employee)
	{
		...
		create.addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				Decree step = new Decree();
				step.setId("0");
				step.setEmployee(employee);
				
				canvas.getChildren().clear();
				
				DecreeForm form = new DecreeForm(step);
							...
							for(Object object:grid.getRows().getChildren())
							{
								...
								if(row.getFirstChild() instanceof Checkbox)
								{
									...
									if(check.isChecked())
									{
										...
										if(on != null)
										{
											Iterator<Decree> iterator = on.getDecrees().iterator();
											while (iterator.hasNext())
											{
												Decree decree = (Decree) iterator.next();
												if(decree.getId().equals(RowUtils.id(row)))
													iterator.remove();
											}
	...
	private void initGrid(Employee employee)
	{
		...
		grid.getColumns().appendChild(new Column("DESKRIPSI",null,"150px"));
		...
		for(Decree fam:service.find(employee.getId()).getDecrees())
		{
			A com = new A();
			...
			row.addEventListener(Events.ON_DOUBLE_CLICK, new EventListener<Event>()
			{
				...
				{
					canvas.getChildren().clear();
					DecreeForm form = new DecreeForm(fam);
					form.setCanvas(canvas);
