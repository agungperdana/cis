...
import com.shara.hr.ui.form.FamilyForm;

/**
 * @author Agung Dodi Perdana
 * @email agung.dodi.perdana@gmail.com 
 */
public class FamilyView extends DashboardContent
{
	...
	public FamilyView(Employee employee)
	{
		super();
	...
	private void initToolbar(Employee employee)
	{
		...
		create.addEventListener(Events.ON_CLICK, new EventListener<Event>()
		{
			...
			public void onEvent(Event arg0) throws Exception
			{
				...
				FamilyForm form = new FamilyForm(family);
							...
							for(Object object:grid.getRows().getChildren())
							{
								...
								if(row.getFirstChild() instanceof Checkbox)
								{
									...
									if(check.isChecked())
									{
										...
										if(on != null)
										{
											Iterator<Family> iterator = on.getFamilys().iterator();
											while (iterator.hasNext())
											{
												Family family = (Family) iterator.next();
												if(family.getId().equals(RowUtils.id(row)))
													iterator.remove();
											}
	...
	private void initGrid(Employee employee)
	{
		...
		grid.getColumns().appendChild(new Column("TIPE",null,"100px"));
		grid.getColumns().appendChild(new Column("NAMA",null,"250px"));
		grid.getColumns().appendChild(new Column("TEMPAT LAHIR",null,"150px"));
		grid.getColumns().appendChild(new Column("TANGGAL LAHIR",null,"125px"));
		grid.getColumns().appendChild(new Column());
		grid.getColumns().getLastChild().setVisible(false);
		grid.setSpan("1");
		
		for(Family fam:service.find(employee.getId()).getFamilys())
		{
			A com = new A();
			...
			row.appendChild(new Label(fam.getType()!=null?fam.getType().name():""));
			row.appendChild(new Label(fam.getName()));
			row.appendChild(new Label(fam.getBirthPlace()));
			row.appendChild(new Label(format.format(fam.getBirthDate())));
			...
			row.addEventListener(Events.ON_DOUBLE_CLICK, new EventListener<Event>()
			{
				...
				{
					canvas.getChildren().clear();
					FamilyForm form = new FamilyForm(fam);
					form.setCanvas(canvas);
